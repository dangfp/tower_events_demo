...
class TodoTest < ActiveSupport::TestCase
  ...
  test 'should create an event after create a new todo' do
    assert_difference('Event.count') do
      Todo.create(project_id: @project.id, name: 'create_todo', creator_id: @current_user.id,  status: 'status_fresh')
    end
  end

  test 'the ancestor and resource of todo event both should be the project' do
    todo  = Todo.create(project_id: @project.id, name: 'new_todo', creator_id: @current_user.id,  status: 'status_fresh')
    event = Event.last
  ...
  test 'the priority and tag of todo event both should be in the data' do
    todo  = Todo.create(project_id: @project.id, name: 'new_todo', creator_id: @current_user.id,  status: 'status_fresh', priority: '!!!', tag: 'test')
    event = Event.last
  ...
  test 'should create an event after change todo status' do
    @todo.update(status: "status_start")
    event = Event.last
